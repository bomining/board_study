...
//		파라미터 이름을 키로 파라미터에 해당하는 파일 정보를 값으로 하는 Map 가져오기
		Map<String, MultipartFile> files = multiRequest.getFileMap();
		
//		files.entrySet() 의 요소 읽어오기
		...
		while (iterator.hasNext()) {
			...
			if (fileSize > 0) {
				String filePath = "C:/Users/KBM/Desktop/Study/";
				
//				파일 업로드 경로 + 현재 년 월 (월별관리)
				filePath = filePath + File.separator + String.valueOf(calendar.get(Calendar.YEAR)) + File.separator + String.valueOf(calendar.get(Calendar.MONTH) + 1);
...
//				saveFile 이 파일이면 true, 아니면 false
